...
build-docs:
	...
	--no-install --no-post-install --pkgs $(PACKAGE-NAME)

# Primarily for day-to-day dev.
# Build libraries from source, build docs (if any), and check dependencies.
build-all:
	raco setup $(DEPS-FLAGS) --pkgs $(PACKAGE-NAME)
...
test:
	raco test -exp $(PACKAGE-NAME)

test-with-errortrace:
	racket -l errortrace -l racket -e '(require (submod "tests/$(PACKAGE-NAME).rkt" test))'
